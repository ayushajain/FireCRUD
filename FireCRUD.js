...
class FireCRUD {
    ...
    editClients(button){
        ...
        if ($(button).attr('id').indexOf("edit") > -1) {
            if (crud.useModal) {
                ...
                for (var prop in crud.columns) {
                    if(crud.columns[prop].type == "input")
                        ...
                        userInfo += "<tr><td>" + crud.columns[prop].title + ": </td><td><select id='" + prop + "'>";
            ...
            //WARNING: nothing in this loop will run if isModal is true
    ...
    createRow(key, readOnly, snapval) {
        ...
        if (readOnly) {
            ...
            if (this.useModal)
                newTableRow += "<button id='edit-" + key + "' data-toggle='modal' data-target='#modal' class='btn btn-primary editbtn'>Edit</button>";
            else
                newTableRow += "<button id='edit-" + key + "' class='btn btn-primary editbtn'>Edit</button>";
